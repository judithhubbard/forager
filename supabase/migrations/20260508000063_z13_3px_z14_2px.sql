-- Tighten z13 + z14 grids further. With 12px markers, the
-- visible-color fraction at each cell-distance:
--   1px overlap: 1/12 visible (~8%)
--   2px overlap: 2/12 visible (~17%)
--   3px overlap: 3/12 visible (~25%)
-- Anything inside 25% visible color we count as "essentially
-- obscured" — the bottom pin's species is hard to read against
-- the top pin's larger swatch.
--
-- z13: 3px cell ≈ 42m
-- z14: 2px cell ≈ 15m
-- z15-17: 1px (strict fully-obscured rule unchanged)
...
as $$
  with cell_size as (
    select case
      ...
      when p_zoom = 15  then 0.0000332        -- 1px@z15
