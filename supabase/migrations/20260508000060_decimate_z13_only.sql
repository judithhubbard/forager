-- Stop decimating at zoom ≥ 14. The user's consistent ask is "show
-- every pin I can interact with" — decimation at any interactable
-- zoom keeps surfacing the "tree X is in this cell, tree Y won the
-- hash" surprise. At zoom 14+ the canvas renderer handles 5-15k
-- markers fine; let it.
--
-- Decimation stays at zoom ≤ 13 where the alternative is rendering
-- 50k+ markers in a continental viewport — that's where the
-- compute-saving argument actually matters.
--
-- Cell sizes:
--   zoom ≤ 12: 360m
--   zoom = 13: 180m
--   zoom ≥ 14: 0 (no decimation)
...
as $$
  with cell_size as (
    select case
      ...
      else 0.0                               -- zoom 14+: no decimation
    end as deg
  ),
  candidates as (
    select p.*,
