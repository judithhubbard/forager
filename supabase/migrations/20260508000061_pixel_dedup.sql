-- Drop pins that share a screen pixel with another pin (i.e., "fully
-- obscured" — same map coordinate when projected to the user's
-- current zoom). Final answer to the long pin-decimation thread:
-- with uniform-size markers, fully-obscured-by-others reduces to
-- "is at the same pixel as another pin." Anything farther apart
-- has at least one visible pixel of its own color, so it must
-- render to be honest.
--
-- Cell sizes = 1 screen pixel in degrees at 42° latitude:
--   z13: 0.000126°    z14: 0.000067°
--   z15: 0.0000332°   z16: 0.0000166°
--   z17: 0.0000084°   z18+: 0 (no dedup; pins are sub-meter apart)
--
-- Performance: this is approximately the migration 52 grid (which
-- the user found "too aggressive" — but only because at the time
-- the cell-boundary decimation was hiding visible-but-adjacent pins.
-- With this precision exactly matching one screen pixel, no visible
-- pin is ever hidden. Only true overlap-at-same-pixel duplicates
-- get dropped — typically 5-15% of pins in dense imports.
