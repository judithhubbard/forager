-- Restore the marker-sized grid (one cell ≈ one pin marker, ≈12px).
-- Migration 56 doubled cell sizes to fit under the 1000-row cap;
-- the client now paginates so the cap doesn't bind, and we can
-- afford the finer grid → 4× more pins per zoom, every
-- visually-distinguishable pin gets drawn.
