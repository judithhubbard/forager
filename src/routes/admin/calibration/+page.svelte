<script lang="ts">
  ...
  // Complex-membership map keyed by scientific_name → array of complexes.
  // Generated by species-complex-unify.cjs into static/species-complexes.json
  // each pipeline run.
  type Mechanism = 'heat_driven' | 'cool_night' | 'frost_anchored' | 'photoperiod' | 'rain_flush' | 'dormancy_break' | 'indeterminate';
  type ComplexEntry = {
    name: string;
    members: string[];
    stage: string;
    mechanism?: Mechanism;
    shift_per_half_zone?: number;
  };
  /** Friendly labels + colors per mechanism. Used by the chip + filter
   *  + tooltips. Colors are WCAG-AA legible on the light viewer bg. */
  const MECHANISM_LABEL: Record<Mechanism, string> = {
    heat_driven: 'Heat-driven',
    cool_night: 'Cool-night',
    frost_anchored: 'Frost-anchored',
    photoperiod: 'Photoperiod',
    rain_flush: 'Rain-flush',
    dormancy_break: 'Dormancy-break',
    indeterminate: 'Indeterminate'
  };
  const MECHANISM_HELP: Record<Mechanism, string> = {
    heat_driven: 'Warmer zones ripen earlier (apples, most fruit). Linear slope.',
    cool_night: 'Cool autumn nights trigger fruiting — warmer zones lag (late-fall mushrooms).',
    frost_anchored: 'Peak tracks first-fall-frost or last-spring-frost; ignores the linear slope.',
    photoperiod: 'Day-length-locked — same DOY across zones.',
    rain_flush: 'Triggered by rain regardless of season.',
    dormancy_break: 'Spring tracks chill-hour completion + warm-up; complicated math.',
  ...
  $: currentComplexes = (() => {
    if (!currentSpecies) return [] as Array<{ name: string; stage: string; members: Array<{ scientific_name: string; common_name: string; review_status: ReviewStatus; is_current: boolean }> }>;
    const cxs = complexesByScientific[currentSpecies.scientific_name] ?? [];
    return cxs.map((cx) => ({
      name: cx.name,
      stage: cx.stage,
      members: cx.members.map((sci) => {
        const sp = species.find((s) => s.scientific_name === sci);
        const status = sp ? (speciesSummaryById[sp.id]?.review_status ?? 'unreviewed') : 'unreviewed';
        return {
          scientific_name: sci,
          common_name: sp?.common_name ?? sci,
          review_status: status,
          is_current: sci === currentSpecies?.scientific_name
        };
      })
    }));
  })();
  ...
  /** Mechanism filter — when set, only species with at least one
   *  complex entry of this mechanism are shown. Empty string =
   *  no filter. */
  let mechanismFilter: Mechanism | '' = '';

  /** Return the dominant mechanism for a species (the mechanism of its
   *  first complex entry; defaults to heat_driven when species is in
   *  no complex). Used by the chip, filter, and sidebar grouping. */
  function mechanismOf(scientificName: string): Mechanism {
    const cxs = complexesByScientific[scientificName] ?? [];
    if (cxs.length === 0) return 'heat_driven';
    return cxs[0].mechanism ?? 'heat_driven';
  }
  ...
  $: filteredSpecies = (() => {
    ...
    if (mechanismFilter) {
      list = list.filter((s) => mechanismOf(s.scientific_name) === mechanismFilter);
    }
    return list;
  })();
  ...
  /** Sidebar grouping mode: status / complex / mechanism. Status groups
   *  by review_status (default). Complex groups by species-complex.
   *  Mechanism groups by biological driver of timing — lets JK scan
   *  the catalog by heat_driven vs cool_night vs frost_anchored etc.
   *  and spot mis-classifications. */
  let sidebarMode: 'status' | 'complex' | 'mechanism' = 'status';
  ...
  /** Sidebar grouped by biological mechanism. Mechanism order is
   *  chosen so heat_driven (the bulk of the catalog) comes first
   *  and the more interesting outliers float to the top of their
   *  groups. */
  const MECHANISM_ORDER: Mechanism[] = [
    'heat_driven', 'cool_night', 'frost_anchored', 'photoperiod',
    'rain_flush', 'dormancy_break', 'indeterminate'
  ];
  $: sidebarByMechanism = (() => {
    const groups: Record<Mechanism, typeof species> = {
      heat_driven: [], cool_night: [], frost_anchored: [],
      photoperiod: [], rain_flush: [], dormancy_break: [],
      indeterminate: []
    };
    for (const sp of species) {
      const m = mechanismOf(sp.scientific_name);
      groups[m].push(sp);
    }
    for (const k of Object.keys(groups) as Mechanism[]) {
      groups[k].sort((a, b) => (a.common_name || '').localeCompare(b.common_name || ''));
    }
    return MECHANISM_ORDER
      .map((m) => ({ mechanism: m, members: groups[m] }))
  ...
  {:else}
    ...
    <aside class="species-sidebar" aria-label="Species list">
      <div class="sb-mode-toggle" title="Switch sidebar grouping">
        ...
        <button
          class="sb-mode-btn"
          class:active={sidebarMode === 'mechanism'}
          on:click={() => (sidebarMode = 'mechanism')}
          title="Group by biological mechanism (heat-driven / cool-night / frost-anchored / …)"
        >Mechanism</button>
        <button
          class="sb-mode-btn"
      ...
      {#if sidebarMode === 'status'}
        {#each [['needs_work','⚠ Needs work'], ['unreviewed','◌ Not reviewed'], ['confirmed','✓ Confirmed']] as [k, label]}
          ...
          {/if}
        {/each}
      {:else if sidebarMode === 'mechanism'}
        {#each sidebarByMechanism as g (g.mechanism)}
          <div class="sb-group">
            <div class="sb-heading sb-mech-heading" title={MECHANISM_HELP[g.mechanism]}>
              <span class="mech-chip mech-{g.mechanism}">{MECHANISM_LABEL[g.mechanism]}</span>
              <span class="sb-count">{g.members.length}</span>
            </div>
    ...
    <div class="picker">
      ...
      <select
        ...
        title="Filter species list by biological mechanism"
      >
        <option value="">All mechanisms</option>
        {#each MECHANISM_ORDER as m}
          <option value={m}>{MECHANISM_LABEL[m]}</option>
        {/each}
      </select>
    ...
    {#if currentSpecies}
      ...
      <div class="species-head">
        ...
        <span
          class="mech-chip mech-{mechanismOf(currentSpecies.scientific_name)}"
          title={(() => {
            const mech = mechanismOf(currentSpecies.scientific_name);
            const sh = complexesByScientific[currentSpecies.scientific_name]?.[0]?.shift_per_half_zone;
          ...
          {#each currentComplexes as cx}
            ...
            <div class="complex-note" title="Members of this complex share a single calibrated harvest curve via species-complex-unify.cjs">
              <div class="complex-head">
                ...
                <span class="complex-stage">({cx.stage})</span>
                <span class="complex-stats">
                  {confirmedCount}/{cx.members.length} confirmed
                </span>
              </div>
              <div class="complex-members">
                {#each cx.members as m}
                  {#if m.is_current}
                    <span class="cm-self" title="this species">▸ {m.common_name}</span>
                  {:else}
                    <button
                      class="cm-link cm-status-{m.review_status}"
                      on:click={() => {
                        const sp = species.find(s => s.scientific_name === m.scientific_name);
                        if (sp) pickSpecies(sp.id);
                      }}
                      title="Click to view {m.common_name} ({m.review_status.replace('_',' ')})"
...
<style>
  ...
  .complex-head { display: flex; align-items: center; gap: 0.4rem; flex-wrap: wrap; }
  .complex-head strong { color: #2a4a2a; }
  .complex-stage { color: #6b7a6b; font-size: 0.78rem; }
  .complex-stats {
    margin-left: auto; font-size: 0.78rem; color: #2a6f2a;
    font-variant-numeric: tabular-nums;
  }
  .complex-members {
    margin-top: 0.4rem;
  ...
  .cm-self {
    font-size: 0.78rem; padding: 0.15rem 0.45rem; color: #1f2a1f;
    background: #d8eacc; border-radius: 0.25rem; font-weight: 600;
  }
  .cm-link {
    font-size: 0.78rem; padding: 0.15rem 0.45rem;
    background: white; border: 1px solid #c7d0c7; border-radius: 0.25rem;
    cursor: pointer; font-family: inherit; color: #1f2a1f;
  }
  .cm-link:hover { background: #f0f5ec; }
  ...
  .src-count, .review-chip, .smoothing-chip, .inat-ghost-chip, .mech-chip {
    font-size: 0.75rem;
    border-radius: 1rem;
  ...
  /* Mechanism chip — one color per biological-driver bucket. Used
     on the species head, in sidebar 'Mechanism' grouping headers,
     and (planned) in per-zone tooltips. All colors picked for
     WCAG-AA legibility on the light viewer background. */
  .mech-chip { font-weight: 500; }
  .mech-chip.mech-heat_driven    { background: #fdebc8; color: #7a3e00; border-color: #d99f3c; }
  .mech-chip.mech-cool_night     { background: #d9e7f4; color: #1c3f6b; border-color: #5d8ab8; }
  .mech-chip.mech-frost_anchored { background: #d4eef0; color: #1c4e54; border-color: #4f9fa5; }
  .mech-chip.mech-photoperiod    { background: #e8def3; color: #44245d; border-color: #8b6fb3; }
  ...
  /* When the sidebar is in 'mechanism' grouping mode the heading
     is itself a mech-chip; the surrounding flex container needs
     to give the chip and count comfortable spacing. */
  .sb-mech-heading {
    display: flex;
    align-items: center;
    gap: 0.4rem;
